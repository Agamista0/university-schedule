...
export default function page() {
  const [scheduleFetchedData, setScheduleFetchedData] = React.useState([]);
  const [loading, setLoading] = React.useState(true);

  React.useEffect(() => {
    const fetchLectures = async () => {
      try {
        const response = await axios.get("/api/distribute-sections");
        if (response.data) {
          setScheduleFetchedData(response.data);
          setLoading(false);
        } else {
          setScheduleFetchedData([]);
          setLoading(false);
        }
      } catch (error) {
      ...
      <div className="ml-64">
        ...
        <div className=" mx-auto px-4 sm:px-6 lg:px-8 ">
          <div className="mb-8">
            ...
            ) : scheduleFetchedData.length > 0 ? (
              <>
                <ScheduleTable
                  scheduleData={scheduleFetchedData}
                  type="section"
                />
                <div
