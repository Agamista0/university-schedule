...
const distributeLectures = async (schedule, registeredStudents, lectureHalls, labs, bufferPercentage = 0.15) => {
    ...
    for (const lectureData of lecturesWithAttendance) {
        ...
        const availableRooms = lectureHalls.filter(hall => hall.center === center && isRoomAvailable(bookedRooms, lectureDay, lectureTime, hall.name));
        ...
        if (roundedAttendance > 0) {
            const availableLabs = labs.filter(lab => lab.center === center && isRoomAvailable(bookedRooms, lectureDay, lectureTime, lab.name));
